...
export function ResearcherSession({children}:{children:React.ReactNode}){
 const [address,setAddress]=useState<string|null>(null),[chainOk,setChainOk]=useState(false),[busy,setBusy]=useState(false),[disconnected,setDisconnected]=useState(false);
 const refresh=async()=>{const p=window.ethereum;if(!p||disconnected){setAddress(null);setChainOk(false);return} const a=await p.request({method:"eth_accounts"}); const c=await p.request({method:"eth_chainId"}); setAddress(a?.[0]||null);setChainOk(parseInt(c,16)===CHAIN_ID)};
 useEffect(()=>{refresh();const p=window.ethereum;if(!p?.on)return;const h=()=>refresh();p.on("accountsChanged",h);p.on("chainChanged",h);return()=>{p.removeListener?.("accountsChanged",h);p.removeListener?.("chainChanged",h)}},[disconnected]);
 ...
 const connect=async()=>{const p=window.ethereum;if(!p)throw new Error("Install an injected EIP-1193 wallet");setBusy(true);setDisconnected(false);try{const a=await p.request({method:"eth_requestAccounts"});setAddress(a?.[0]||null);await switchNetwork()}finally{setBusy(false)}};
 const disconnect=async()=>{const p=window.ethereum;setBusy(true);setDisconnected(true);try{await p?.request({method:"wallet_revokePermissions",params:[{eth_accounts:{}}]})}catch{}finally{setAddress(null);setChainOk(false);setBusy(false)}};
