...
type Context={address:string|null;chainOk:boolean;busy:boolean;connect:()=>Promise<void>;switchNetwork:()=>Promise<void>};
...
export function ResearcherSession({children}:{children:React.ReactNode}){
 ...
 const value=useMemo(()=>({address,chainOk,busy,connect,switchNetwork}),[address,chainOk,busy]);return <C.Provider value={value}>{children}</C.Provider>
}
